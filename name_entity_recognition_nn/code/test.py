...
def prepare_for_token_eval(y_pred, tags_test, replacement='O'):
    ...
    # remove PAD in y_pred_label according to y_truth
    ...
    # calibration: replace PAD by O in y_pred_label_reduced
    ...
    for i in range(len(tags_test)):
        ...
        for j in range(len(tags_test[0])):
            if tags_test[i][j] != config.TAG2ID['PAD']:
                ...
                # calibration: replace PAD by O in y_pred
...
def main():
    ...
    model_new = tf.keras.models.load_model(config.MODEL_PATH, custom_objects={'CRFModel':CRFModel})
    ...
    if mode == 1:
        ...
        t3 = time.time()
        y_pred_tk, y_truth_tk = prepare_for_token_eval(y_pred[1], y_test)
        ...
        t5 = time.time()
        res_pred, res_truth = prepare_for_tag_eval(y_pred[1], y_test)
