...
#         print('------------------------------')
        ...
        print(classification_report(self.y_true, self.y_score, target_names=self.labels, zero_division=0))
        print()
    
        if config.EXCEPTION is not None:
            result = classification_report(self.y_true, self.y_score, target_names=self.labels, zero_division=0, output_dict=True)
            exception = config.EXCEPTION
            precisions = []
            recalls = []
            f1s = []
            for lb in set(result.keys())-set(['accuracy', 'macro avg', 'weighted avg', exception]):
                precisions.append(result[lb]['precision'])
                recalls.append(result[lb]['recall'])
                f1s.append(result[lb]['f1-score'])
            print('Macro avg Precision (exclude): '  ,sum(precisions)/len(precisions))
            print('Macro avg Recall (exclude): '  ,sum(recalls)/len(recalls))
            print('Macro avg F1 (exclude): '  ,sum(f1s)/len(f1s))
