...
# Test set
TEST_TABLE_PATH = '../../TableQA-master/test/test.tables.json'
TEST_DATA_PATH = '../../TableQA-master/test/test.json'
...
INPUT_MODEL_PATH = '../model/m1_ep30.h5'   
...
def get_model(num_sel_agg,num_cond_op,num_cond_conn_op):
    ...
    bert_model = load_trained_model_from_checkpoint(BERT_PATH.config, BERT_PATH.checkpoint, seq_len=None)
...
def main():
    ...
    # load data
    test_tables = read_tables(TEST_TABLE_PATH)
    test_data = read_data(TEST_DATA_PATH, test_tables)
    ...
    with open(TASK1_PREDICTION, 'w') as f:
