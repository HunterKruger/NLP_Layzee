...
# model path
OUTPUT_MODEL_PATH = '../model/m1_ep30.h5'        # save weight path
INPUT_MODEL_PATH = '../model/m1_ep10.h5'         # load weight path

# history log
HISTORY_PATH = '../history/m1_ep30.csv'
...
class EvaluateCallback(Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        # Example:  SELECT SUM(district) FROM table WHERE name='swag' AND population>100

        # validation metrics
        print('total_acc: {}'.format(all_correct / num_queries))                   # conn_acc, agg_acc, conds_acc all correct
        print('agg_acc: {}'.format(agg_correct / num_queries))                     # 'SUM(district)' in example
        print('conn_acc: {}'.format(conn_correct / num_queries))                   # 'AND' in example 
        print('conds_acc: {}'.format(conds_correct / num_queries))                 # (name, =) & (population, >) in example
        print('conds_col_id_acc: {}'.format(conds_col_id_correct / num_queries))   # name & population in example
        
        # validation metrics for logging
        logs['val_total_acc'] = all_correct / num_queries     
        logs['val_agg_acc'] =  agg_correct / num_queries     
        logs['val_conn_acc'] = conn_correct / num_queries
        logs['val_conds_acc'] = conds_correct / num_queries
        logs['val_conds_col_id_acc'] = conds_col_id_correct / num_queries


def get_model(num_sel_agg,num_cond_op,num_cond_conn_op, path=BERT_PATH):

    bert_model = load_trained_model_from_checkpoint(path.config, BERT_PATH.checkpoint, seq_len=None)
    ...
    # predict cond_conn_op (operator between conditions)
    ...
    # predict sel_agg (which columns in SELECT and their AGG)
    ...
    # predict cond_op (which columns in condition and their operaters)
...
def main():
    ...
    token_dict = load_vocabulary(BERT_PATH.vocab)
    ...
    model = get_model(num_sel_agg,num_cond_op,num_cond_conn_op, BERT_PATH)
    ...
    if NUM_GPUS > 1:
        ...
        para_model.compile(loss='sparse_categorical_crossentropy', optimizer=RAdam(lr=LR))   
    else:
        print('using single gpu or cpu'.format(NUM_GPUS))
        model.compile(loss='sparse_categorical_crossentropy', optimizer=RAdam(lr=LR))  
    ...
    callbacks = [
        ...
        EarlyStopping(monitor='val_total_acc', patience=3, verbose=0, mode='max', restore_best_weights=True)
    ]

    if NUM_GPUS > 1:
        history = para_model.fit_generator(train_dataseq, epochs=EPOCHS, callbacks=callbacks)
    else:
        history = model.fit_generator(train_dataseq, epochs=EPOCHS, callbacks=callbacks)
