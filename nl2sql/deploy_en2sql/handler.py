...
class English2SQL(BaseHandler, ABC):
    """
    Transformers text classifier handler class. This handler takes a text (string) and
    as input and returns the classification text based on the serialized transformers checkpoint.
    """
    def __init__(self):
        super(English2SQL, self).__init__()
    ...
    def initialize(self, ctx):
        ...
        # model_dir = '/home/fengyuan/workspaceGServer/NLP/SPD_BANK/deploy_en2sql/t5-base-finetuned-wikiSQL'
    ...
    def inference(self, inputs):
        ...
        # logger.info(inputs[0].get('body').decode('utf-8'))  # str

        json_ = inputs[0].get('body')
        query = json_['query']
        table_name = json_['tableName']
        prediction = self.text2sql(query)
        prediction = prediction.replace('table', table_name)
