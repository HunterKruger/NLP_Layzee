...
def encode_one_sample(sentence,
                      max_len=config.MAX_LEN, 
                      model_name=config.BASE_MODEL_PATH):  
    
    tokenizer = BertTokenizer.from_pretrained(model_name)
    encoded_sent = tokenizer(sentence,
                             padding='max_length',     # Pad to max_length
                             truncation=True,          # Truncate to max_length
                             max_length=max_len,       # Set max_length
                             return_tensors='tf')  
    token_ids = tf.squeeze(encoded_sent['input_ids'])            
    attention_masks = tf.squeeze(encoded_sent['attention_mask'])
    token_type_ids = tf.squeeze(encoded_sent['token_type_ids']) 

    return [tf.convert_to_tensor([token_ids]), tf.convert_to_tensor([attention_masks]),tf.convert_to_tensor([token_type_ids])]
