...
class CustomDataset(tf.keras.utils.Sequence): 
    ...
    def __init__(self,
                 ...
                 word2vec_path=config.WORD2VEC_FILE,
        ...
        self.word2vec = self.load_embedding(word2vec_path) 
    ...
    def __getitem__(self, idx): # get tokenized sample by index
        ...
        sents = sents.apply(lambda x: self.segmentation(x))
        ...
        if self.labels is None:
            ...
            labels = labels.apply(lambda x: segmentation(x))
...
# def process_text_by_embedding(sents, word2vec, maxlen=config.MAX_LEN):
...
#     sents = sents.apply(lambda x: tokenize(x))
#     sents = sents.tolist()
#     sents = tf.keras.preprocessing.sequence.pad_sequences(sents, value=word2vec.key_to_index['<PAD>'], padding='post', maxlen=maxlen)
#     return sents
    
    
# def load_embedding(path=config.EMBEDDING_PATH):
#     word2vec = gensim.models.KeyedVectors.load_word2vec_format(path, binary=False)
#     word2vec.add_vector('<PAD>', (np.random.rand(len(word2vec[0])) - 0.5) * 2)
#     word2vec.add_vector('<OOV>', (np.random.rand(len(word2vec[0])) - 0.5) * 2)
#     return word2vec